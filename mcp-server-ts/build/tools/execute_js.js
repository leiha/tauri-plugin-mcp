...
export function registerExecuteJsTool(server) {
    server.tool("execute_js", "Executes arbitrary JavaScript in a webview. Returns the result of the last statement or promise resolution. This is the universal escape hatch — use it for anything not covered by other tools. For Tauri event system access (emit, listen, sniff), use `window.__TAURI__` APIs directly. Caution: can modify page state.", {
        ...
        try {
            ...
            return createErrorResponse(`Failed to execute JavaScript: ${error.message}`);
        }
    });
}
