 ...
 */
const WINDOW = z.string().default("main").describe("Webview label. 'main' is the application window; a tab or child webview has its own label " +
    "(e.g. 'lightbox-0' in oshun). Use list_windows or the host app's own listing to find it.");
const TIMEOUT = z.number().int().positive().optional().describe("Milliseconds to wait for the page to answer. Default 8000. A page that never ran the probe " +
    "answers 'probe-absent' immediately rather than waiting this out.");
async function forward(command, params, emptyHint) {
    try {
        ...
        return createSuccessResponse(isEmpty ? emptyHint : formatResultAsText(payload));
    }
    catch (error) {
        return createErrorResponse(`${command}: ${error instanceof Error ? error.message : String(error)}`);
    }
}
export function registerInspectPageTools(server) {
    server.tool("get_console", "Console messages of a page — including those emitted BEFORE this call. The hook is installed " +
        ...
        "unhandled-rejection. Use this when a component renders blank or an interaction does nothing.", { window_label: WINDOW, timeout_ms: TIMEOUT }, { title: "Read a page's console", readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false }, async ({ window_label, timeout_ms }) => forward("get_console", { window_label, timeout_ms }, "The page logged nothing. That is a real answer, not a failure: the buffer exists and is empty."));
    server.tool("get_network", "Network calls a page made through fetch or XMLHttpRequest, with method, URL, status and duration. " +
        "Returns [{at, via, method, url, status, ms, failed}]. Use this to check which endpoint was actually " +
        "called, or why a request failed. For resources no hook sees (images, stylesheets), use get_timings.", { window_label: WINDOW, timeout_ms: TIMEOUT }, { title: "Read a page's network calls", readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false }, async ({ window_label, timeout_ms }) => forward("get_network", { window_label, timeout_ms }, "No fetch or XHR call was captured. The page may use only static resources — try get_timings."));
    server.tool("get_timings", "Every resource the page requested, from the browser's own Resource Timing API — images, stylesheets, " +
        "scripts, beacons included. Sees more than get_network but carries NO status code: the two answer " +
        "different questions and both are exposed rather than merged.", { window_label: WINDOW, timeout_ms: TIMEOUT }, { title: "Read a page's resource timings", readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false }, async ({ window_label, timeout_ms }) => forward("get_timings", { window_label, timeout_ms }, "No resource timing recorded."));
    server.tool("get_page_snapshot", "URL, title, console, network and resource timings of a page in ONE round trip. Prefer this over " +
        "three separate calls when diagnosing: the cookie reply channel is chunked, so one call is markedly " +
        "faster than three.", { window_label: WINDOW, timeout_ms: TIMEOUT }, { title: "Snapshot a page's runtime state", readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false }, async ({ window_label, timeout_ms }) => forward("get_page_snapshot", { window_label, timeout_ms }, "Empty snapshot."));
    server.tool("inspect_dom", "The rendered DOM of any page, including one the app does not own — the foreign-page counterpart of " +
        "get_dom. Returns the current outerHTML, after scripts have run, not the served source. Large pages " +
        "arrive in cookie chunks, so this is slower than get_dom; use get_dom on pages the app owns.", { window_label: WINDOW, timeout_ms: TIMEOUT }, { title: "Read the rendered DOM of any page", readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false }, async ({ window_label, timeout_ms }) => forward("inspect_dom", { window_label, timeout_ms }, "Empty DOM."));
    server.tool("inspect_eval", "Evaluates a JavaScript EXPRESSION on any page — including one the app does not own — and returns its " +
        ...
        expression: z.string().describe("A JavaScript expression whose VALUE is returned, e.g. " +
            "`document.querySelectorAll('.row').length` or `(() => { const e = document.querySelector('#x'); " +
            "return e ? e.textContent : null; })()`."),
        window_label: WINDOW,
        timeout_ms: TIMEOUT,
    }, { title: "Evaluate an expression on any page", readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: false }, async ({ expression, window_label, timeout_ms }) => {
    ...
    server.tool("list_commands", "What this bridge can do, straight from the running binary. Returns every command with " +
        ...
        "probeVersion, so an old binary is told apart from a missing feature.", {}, { title: "List the bridge's commands", readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false }, async () => forward("list_commands", {}, "No catalogue — this binary predates list_commands."));
    server.tool("capture_webview", "An IMAGE of what a page actually looks like — including a child webview (a tab), which " +
        ...
        window_label: WINDOW,
        save_path: z.string().optional().describe("Absolute path to write the PNG to. Strongly recommended. Without it the image comes back as a data URI."),
        region: z.enum(["visible", "full"]).optional().describe("'visible' (default) captures the viewport; 'full' captures the whole document — often taller, " +
    ...
    server.tool("inspect_map", "A compact map of what a page offers to interact with — links, buttons, inputs, selects, and " +
        ...
        timeout_ms: TIMEOUT,
    }, { title: "Map a page's interactive elements", readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false }, async (params) => forward("inspect_map", params, "No interactive element found."));
    server.tool("inspect_click", "Clicks an element on any page, with the FULL pointer sequence (pointerdown, mousedown, pointerup, " +
        ...
        timeout_ms: TIMEOUT,
    }, { title: "Click an element on any page", readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: false }, async (params) => forward("inspect_click", params, "Nothing clicked."));
    server.tool("inspect_fill", "Types a value into a field on any page, the way a user would: the native value setter (assigning " +
        ...
        window_label: WINDOW,
        timeout_ms: TIMEOUT,
    }, { title: "Fill a field on any page", readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: false }, async (params) => forward("inspect_fill", params, "Nothing filled."));
    server.tool("inspect_press", "Presses a key on the focused element of any page, or on `selector` when given. Use it for Enter to " +
        ...
        window_label: WINDOW,
        timeout_ms: TIMEOUT,
    }, { title: "Press a key on any page", readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: false }, async (params) => forward("inspect_press", params, "Nothing pressed."));
    server.tool("inspect_scroll", "Scrolls the window of any page, or an element when `selector` is given. Returns the resulting " +
        ...
        timeout_ms: TIMEOUT,
    }, { title: "Scroll any page", readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false }, async (params) => forward("inspect_scroll", params, "Nothing scrolled."));
    server.tool("inspect_wait", "Waits until a selector is visible, attached, or gone on any page. Use it after a click that " +
        ...
        window_label: WINDOW,
        options: z.object({
            state: z.enum(["visible", "attached", "detached"]).optional().describe("'visible' (default) also requires a non-zero box and no display:none/visibility:hidden. " +
    ...
    }, { title: "Wait for an element on any page", readOnlyHint: true, destructiveHint: false, idempotentHint: false, openWorldHint: false }, async (params) => forward("inspect_wait", params, "Condition not reached."));
}
